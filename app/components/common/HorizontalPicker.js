...
import {Platform, ScrollView, StyleSheet, Text, TouchableOpacity, View, ViewPropTypes} from "react-native";
import PropTypes from "prop-types";

const defaultForegroundColor = "#444";
const defaultSelectedItemColor = "#d2ad57";
...
class HorizontalPicker extends Component {
    ...
    getItemWidth = () => {
return this.props.itemWidth && this.props.itemWidth > 0
        ? this.props.itemWidth : defaultItemWidth
};

    getItemHeight = () => {
return this.props.itemHeight && this.props.itemHeight > 0
        ? this.props.itemHeight : defaultItemHeight
};

    getComponentWidth = () => {
return this.state.bounds ? this.state.bounds.width : 0
};
    ...
    getIndexForValue = (item, children = this.getChildren()) => {
        const index = children.findIndex((e) => {
return e.props.value === item
});
...
return React.Children.toArray(children)
    ...
    renderChild = (child) => {
        ...
        const selected = itemValue === this.props.selectedValue;
        const color = selected ? this.props.selectedItemColor : this.props.foregroundColor || defaultForegroundColor;
        return (
            <TouchableOpacity key={itemValue} style={{width: this.getItemWidth(), height: this.getItemHeight()}}
                              onPress={this.handleItemPress.bind(this, itemValue)}>
                <View style={[styles.itemContainer, {width: this.getItemWidth(), height: this.getItemHeight()}]}>
                    <Text style={[styles.itemText, child.props.style, {
                        color,
                        fontWeight: selected ? "bold" : "400",
                        fontSize: selected ? 21 : 16
