...
import route_config from "../../../navigators/routeConfig";
...
export default class DrawerLayout extends Component {
	...
	componentWillMount() {
		...
		openValue.addListener(({value}) => {
			...
			if (drawerShown !== this.state.drawerShown) {
				this.setState({drawerShown, accessibilityViewIsModal: drawerShown});
			}
	...
	render() {
		 ...
		 **/
		let dynamicDrawerStyles = {}, animatedDrawerStyles = {};
		let outputRange;
		...
		const overlayOpacity = openValue.interpolate({
			inputRange: [0, 1],
			outputRange: [0, this.props.drawerOpacity],
			extrapolate: "clamp"
		});
			...
			transform: [
				{
					translateX: openValue.interpolate({
						inputRange: [0, 1],
						outputRange: [0, this.props.drawerWidth]
					})
				}
			],
			flex: 1,
		...
		return (
			<View
				...
				{...this._addPanHandler()}
			>
				<Animated.View style={[styles.main, left]}>
					<TouchableWithoutFeedback
	...
	_addPanHandler = () => {
		// // console.log(this._drawerComponentIsShowing());
		if (this._drawerComponentIsShowing()) {
			return this._panResponder.panHandlers;
		}
	};

	_drawerComponentIsShowing = () => {
		if (this.props.children.props.navigation.state.routes[0] && this.props.children.props.navigation.state.routes[0].routes.length > 0) {
			const {routes} = this.props.children.props.navigation.state.routes[0];
			const lastRoute = routes[routes.length - 1];
			// // console.log(lastRoute);

			if (lastRoute.params && lastRoute.params.type === "REPLACE") {
				return route_config().ROUTE_PAGES.filter((routeC) => {
					if (routeC.drawer_component) {
						return routeC.routeName === lastRoute.routeName;
					}
					return false;
				}).length > 0;
			}
